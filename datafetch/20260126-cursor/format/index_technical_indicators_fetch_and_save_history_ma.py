"""
独立接口程序：获取并存储历史分时MA数据。

本程序为 index_technical_indicators_sync.py 中历史分时MA接口的独立版本，可独立运行。
支持与其他接口程序并发运行。
...
logger = get_logger("format.index_technical_indicators_fetch_and_save_history_ma")
...
def main() -> None:
    """
    主函数：通过查询数据库股票列表，调用历史分时MA接口获取并存储数据。
