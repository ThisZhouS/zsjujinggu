"""
独立接口程序：获取并存储近年业绩预告数据。

本程序为 financial_core_indicators_sync.py 中近年业绩预告接口的独立版本，可独立运行。
支持与其他接口程序并发运行。
...
logger = get_logger("format.financial_core_indicators_fetch_and_save_performance_forecast")
...
def main() -> None:
    """
    主函数：通过查询数据库股票列表，调用近年业绩预告接口获取并存储数据。
    ...
    logger.info("近年业绩预告累计写入记录数: {}", total)
