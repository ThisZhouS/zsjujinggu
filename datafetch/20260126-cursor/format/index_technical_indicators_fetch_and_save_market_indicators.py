...
本程序为 index_technical_indicators_sync.py 中行情指标接口的独立版本，可独立运行。
支持与其他接口程序并发运行。
...
logger = get_logger("format.index_technical_indicators_fetch_and_save_market_indicators")
...
def main() -> None:
    """
    主函数：通过查询数据库股票列表，调用行情指标接口获取并存储数据。
    ...
    # 不设置开始时间和结束时间，获取全部数据
    start_date: Optional[str] = 20260101
