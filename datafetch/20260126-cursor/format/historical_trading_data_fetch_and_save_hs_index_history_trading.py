"""
独立接口程序：获取并存储HS指数历史分时交易数据。

本程序为 historical_trading_data_sync.py 中HS指数历史分时交易接口的独立版本，可独立运行。
支持与其他接口程序并发运行。
...
logger = get_logger("format.historical_trading_data_fetch_and_save_hs_index_history_trading")
...
def main() -> None:
    ...
    logger.info("HS指数历史分时交易累计写入记录数: {}", total)
