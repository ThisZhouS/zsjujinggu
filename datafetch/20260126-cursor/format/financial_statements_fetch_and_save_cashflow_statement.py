...
本程序为 financial_statements_sync.py 中现金流量表接口的独立版本，可独立运行。
支持与其他接口程序并发运行。
...
logger = get_logger("format.financial_statements_fetch_and_save_cashflow_statement")
...
def main() -> None:
    """
    主函数：通过查询数据库股票列表，调用现金流量表接口获取并存储数据。
    ...
    logger.info("现金流量表累计写入记录数: {}", total)
