"""
独立接口程序：获取并存储指数历史分时BOLL数据。

本程序为 shanghai_shenzhen_technical_indicators_sync.py 中指数历史分时BOLL接口的独立版本，可独立运行。
支持与其他接口程序并发运行。
...
logger = get_logger("format.shanghai_shenzhen_technical_indicators_fetch_and_save_history_boll")
...
def main() -> None:
    """
    主函数：通过查询数据库沪深主要指数列表，调用指数历史分时BOLL接口获取并存储数据。
    ...
    logger.info("指数历史分时BOLL累计写入记录数: {}", total)
