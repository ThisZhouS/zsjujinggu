"""
独立接口程序：获取并存储股票基础信息数据。

本程序为 company_basic_info_sync.py 中股票基础信息接口的独立版本，可独立运行。
支持与其他接口程序并发运行。
...
logger = get_logger("format.company_basic_info_fetch_and_save_stock_basic_info")
...
def main() -> None:
    """
    主函数：通过查询数据库股票列表，调用股票基础信息接口获取并存储数据。
