"""
独立接口程序：获取并存储当天逐笔交易数据。

本程序为 trading_details_special_data_sync.py 中当天逐笔交易接口的独立版本，可独立运行。
支持与其他接口程序并发运行。
...
logger = get_logger("format.trading_details_special_data_fetch_and_save_today_tick_trade")
...
def main() -> None:
    ...
    logger.info("当天逐笔交易累计写入记录数: {}", total)
