...
本程序为 stock_pool_classification_sync.py 中涨停股池接口的独立版本，可独立运行。
支持与其他接口程序并发运行。
...
logger = get_logger("format.stock_pool_classification_fetch_and_save_limit_up_pool")
...
def main(date: Optional[str] = None) -> None:
    ...
    logger.info("开始同步涨停股池数据，日期: {}", date)
