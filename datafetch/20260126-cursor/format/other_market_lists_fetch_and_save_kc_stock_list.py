"""
独立接口程序：获取并存储科创股票列表数据。

本程序为 other_market_lists_sync.py 中科创股票列表接口的独立版本，可独立运行。
支持与其他接口程序并发运行。

API接口: fetch_and_save_kc_stock_list
"""
...
logger = get_logger("format.other_market_lists_fetch_and_save_kc_stock_list")


def main() -> None:
    """
    主函数：调用科创股票列表接口获取并存储数据。
