...
本程序为 financial_quarters_events_sync.py 中近年分红接口的独立版本，可独立运行。
支持与其他接口程序并发运行。
...
logger = get_logger("format.financial_quarters_events_fetch_and_save_recent_dividend")
...
def main() -> None:
    """
    主函数：通过查询数据库股票列表，调用近年分红接口获取并存储数据。
    ...
    # 近年分红接口：股票代码来源于股票列表，股票代码需要去掉 "." 及其后缀
