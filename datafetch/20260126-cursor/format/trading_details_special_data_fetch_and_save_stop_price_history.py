"""
独立接口程序：获取并存储历史涨跌停价格数据。

本程序为 trading_details_special_data_sync.py 中历史涨跌停价格接口的独立版本，可独立运行。
支持与其他接口程序并发运行。
...
logger = get_logger("format.trading_details_special_data_fetch_and_save_stop_price_history")
...
def main() -> None:
    ...
    total = 0
    for code in stock_codes_with_suffix:
        try:
    ...
    logger.info("历史涨跌停价格累计写入记录数: {}", total)
