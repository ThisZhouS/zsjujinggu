"""
独立接口程序：获取并存储公司十大流通股东数据（20260323一期）。

支持两种模式：
- full：全量，不设时间参数。
- incremental：增量，使用当天日期作为开始和结束时间。
...
INTERFACE_NAME = "fetch_and_save_company_top_flow_holders"
...
logger = get_logger("format.20260323一期.shareholder_basic_info_top_flow_holders")
...
def resolve_dates(mode: str, date_str: Optional[str]) -> tuple[Optional[str], Optional[str]]:
    ...
    Args:
        ...
        date_str (Optional[str]): 日期字符串，格式 YYYY-MM-DD。
    ...
    async with semaphore:
        try:
            count = await asyncio.to_thread(
                call_with_retry,
                lambda: client.fetch_and_save_company_top_flow_holders(
            ...
            reason = str(exc)
            logger.error("十大流通股东 - 股票 {} 失败（已重试 {} 次）: {}", stock_code, retry_count, reason)
