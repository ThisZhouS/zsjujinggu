"""
独立接口程序：获取并存储 HS 股票历史交易数据（20260323一期）。
...
INTERFACE_NAME = "fetch_and_save_hs_stock_history_trading"
...
logger = get_logger("format.20260323一期.historical_trading_data_hs_stock_history")
...
async def process_one_stock(
    ...
    """
    异步处理单只股票的历史交易数据获取和存储（含重试）。

    Args:
        client (HistoricalTradingData): API 客户端。
        stock_code (str): 股票代码。
    ...
    async with semaphore:
        try:
            count = await asyncio.to_thread(
                call_with_retry,
                lambda: client.fetch_and_save_hs_stock_history_trading(
                    stock_code, "d", "n", start_date, end_date, None
                ),
                retry_count,
            )
            return count
            ...
            reason = str(exc)
            logger.error("HS历史交易 - 股票 {} 失败（已重试 {} 次）: {}", stock_code, retry_count, reason)
...
async def main() -> None:
    """
    主函数：单线程异步批量同步 HS 股票历史交易数据，附进度条。
