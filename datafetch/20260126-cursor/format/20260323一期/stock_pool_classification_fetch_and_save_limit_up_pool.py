...
INTERFACE_NAME = "fetch_and_save_limit_up_pool"
...
logger = get_logger("format.20260323一期.stock_pool_classification_limit_up_pool")
    ...
    async with semaphore:
        try:
            count = await asyncio.to_thread(
                call_with_retry,
                lambda: client.fetch_and_save_limit_up_pool(target_date),
                retry_count,
            )
            return count
        except Exception as exc:  # pylint: disable=broad-except
            reason = str(exc)
            logger.error("涨停股池 - 日期 {} 失败（已重试 {} 次）: {}", target_date, retry_count, reason)
...
async def main() -> None:
    ...
    with make_progress_bar(target_dates, desc="涨停股池") as pbar:
