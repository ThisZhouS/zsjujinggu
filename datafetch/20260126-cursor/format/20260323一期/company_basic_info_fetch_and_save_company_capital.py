"""
独立接口程序：获取并存储公司股本表数据（20260323一期）。
...
INTERFACE_NAME = "fetch_and_save_company_capital"
...
logger = get_logger("format.20260323一期.company_basic_info_company_capital")
    ...
    async with semaphore:
        try:
            count = await asyncio.to_thread(
                call_with_retry,
                lambda: client.fetch_and_save_company_capital(stock_code, start_date, end_date),
                retry_count,
            )
            return count
        except Exception as exc:  # pylint: disable=broad-except
            reason = str(exc)
            logger.error("公司股本表 - 股票 {} 失败（已重试 {} 次）: {}", stock_code, retry_count, reason)
...
async def main() -> None:
    ...
    with make_progress_bar(stock_codes, desc="公司股本表") as pbar:
        for code in pbar:
