"""
独立接口程序：获取并存储沪深基金列表数据。

本程序为 major_market_lists_sync.py 中沪深基金列表接口的独立版本，可独立运行。
支持与其他接口程序并发运行。

API接口: fetch_and_save_hs_fund_list
"""
...
logger = get_logger("format.major_market_lists_fetch_and_save_hs_fund_list")


def main() -> None:
    """
    主函数：调用沪深基金列表接口获取并存储数据。
