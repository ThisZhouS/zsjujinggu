"""
独立接口程序：获取并存储财务主要指标数据。

本程序为 financial_core_indicators_sync.py 中财务主要指标接口的独立版本，可独立运行。
支持与其他接口程序并发运行。
...
logger = get_logger("format.financial_core_indicators_fetch_and_save_financial_main_indicators")
...
def main() -> None:
    """
    主函数：通过查询数据库股票列表，调用财务主要指标接口获取并存储数据。
