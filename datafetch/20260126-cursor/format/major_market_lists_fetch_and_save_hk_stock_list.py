"""
独立接口程序：获取并存储港股股票列表数据。

本程序为 major_market_lists_sync.py 中港股股票列表接口的独立版本，可独立运行。
支持与其他接口程序并发运行。

API接口: fetch_and_save_hk_stock_list
"""
...
logger = get_logger("format.major_market_lists_fetch_and_save_hk_stock_list")


def main() -> None:
    """
    主函数：调用港股股票列表接口获取并存储数据。
