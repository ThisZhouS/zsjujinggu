...
本程序为 company_basic_info_sync.py 中公司简介接口的独立版本，可独立运行。
支持与其他接口程序并发运行。
...
logger = get_logger("format.company_basic_info_fetch_and_save_company_intro")
...
def main() -> None:
    """
    主函数：通过查询数据库股票列表，调用公司简介接口获取并存储数据。
