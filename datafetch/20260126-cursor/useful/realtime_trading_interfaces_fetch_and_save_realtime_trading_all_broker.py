"""
独立接口程序：获取并存储实时交易数据（全部 券商数据源）。

本程序为 realtime_trading_interfaces_sync.py 中实时交易数据（全部 券商数据源）接口的独立版本，可独立运行。
支持与其他接口程序并发运行。
...
logger = get_logger("format.realtime_trading_interfaces_fetch_and_save_realtime_trading_all_broker")


def main() -> None:
    """
    主函数：调用实时交易数据（全部 券商数据源）接口获取并存储数据。
