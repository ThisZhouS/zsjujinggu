"""
独立接口程序：获取并存储沪深买卖五档盘口数据。

本程序为 market_depth_data_sync.py 中沪深买卖五档盘口接口的独立版本，可独立运行。
支持与其他接口程序并发运行。
...
logger = get_logger("format.market_depth_data_fetch_and_save_hs_stock_real_five")
...
def main() -> None:
    ...
    logger.info("沪深买卖五档盘口累计写入记录数: {}", total)
