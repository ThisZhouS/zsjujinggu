"""
独立接口程序：获取并存储历届董事会成员数据。

本程序为 company_historical_data_sync.py 中历届董事会成员接口的独立版本，可独立运行。
支持与其他接口程序并发运行。
...
logger = get_logger("format.company_historical_data_fetch_and_save_board_member")
...
def main() -> None:
    """
    主函数：通过查询数据库股票列表，调用历届董事会成员接口获取并存储数据。
    ...
    logger.info("历届董事会成员累计写入记录数: {}", total)
