"""
辅助程序：从 2026-02-01 起按日遍历获取并存储强势股池数据。

本程序基于 `format.stock_pool_classification_fetch_and_save_strong_pool`，
通过日期遍历的方式多次调用原始接口程序。
...
from format.stock_pool_classification_fetch_and_save_strong_pool import (  # noqa: E402
...
logger = get_logger("useful.stock_pool_classification_fetch_and_save_strong_pool")
...
def main() -> None:
    """
    主函数：从 2026-02-01 起按日遍历同步强势股池数据，一直遍历到今天。

    API接口: fetch_and_save_strong_pool
    """
    start_date_str, end_date_str = get_full_fetch_date_range()

    logger.info("开始遍历强势股池数据，日期范围: {} 至 {}", start_date_str, end_date_str)
    ...
    for current_date in get_date_range(start_date_str, end_date_str, format_str="%Y-%m-%d"):
        ...
        try:
            logger.info("开始处理强势股池数据，日期: {}", date_str)
            run_once_for_date(date=date_str)
        except Exception as exc:  # pylint: disable=broad-except
            logger.error("强势股池 - 日期 {} 处理失败: {}", date_str, exc)

    logger.info("强势股池数据遍历完成。")
