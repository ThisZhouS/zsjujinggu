"""
辅助程序：在固定时间区间内获取并存储历史分时KDJ数据。

本程序基于 `format.index_technical_indicators_fetch_and_save_history_kdj`，
仅修改时间参数为固定范围：
...
logger = get_logger("useful.index_technical_indicators_fetch_and_save_history_kdj")
...
def main() -> None:
    """
    主函数：通过查询数据库股票列表，在固定时间区间内调用历史分时KDJ接口获取并存储数据。
    ...
    logger.info("历史分时KDJ累计写入记录数: {}", total)
