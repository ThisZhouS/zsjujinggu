"""
独立接口程序：获取并存储根据股票找相关指数、行业、概念数据。

本程序为 index_relationship_mapping_sync.py 中根据股票找相关指数、行业、概念接口的独立版本，可独立运行。
支持与其他接口程序并发运行。
...
logger = get_logger("format.index_relationship_mapping_fetch_and_save_related_codes_by_stock")
...
def main() -> None:
    """
    主函数：通过查询数据库股票列表，调用根据股票找相关指数、行业、概念接口获取并存储数据。
    ...
    # 根据股票找相关指数、行业、概念接口需要不带后缀的股票代码（如 "000001"）
    ...
    logger.info("股票 -> 指数/行业/概念 累计写入记录数: {}", total)
