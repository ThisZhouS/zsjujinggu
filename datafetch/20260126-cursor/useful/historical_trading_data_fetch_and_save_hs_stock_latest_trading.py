"""
独立接口程序：获取并存储HS股票最新分时交易数据。

本程序为 historical_trading_data_sync.py 中HS股票最新分时交易接口的独立版本，可独立运行。
支持与其他接口程序并发运行。
...
logger = get_logger("format.historical_trading_data_fetch_and_save_hs_stock_latest_trading")
...
def main() -> None:
    ...
    logger.info("HS股票最新分时交易累计写入记录数: {}", total)
