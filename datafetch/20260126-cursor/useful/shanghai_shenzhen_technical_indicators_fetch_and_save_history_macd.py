"""
辅助程序：在固定时间区间内获取并存储指数历史分时MACD数据。

本程序基于 `format.shanghai_shenzhen_technical_indicators_fetch_and_save_history_macd`，
仅修改时间参数为固定范围：
...
logger = get_logger(
    "useful.shanghai_shenzhen_technical_indicators_fetch_and_save_history_macd",
)
...
def main() -> None:
    """
    主函数：通过查询数据库沪深主要指数列表，在固定时间区间内调用指数历史分时MACD接口获取并存储数据。
