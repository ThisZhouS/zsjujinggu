...
本程序为 index_relationship_mapping_sync.py 中所属指数接口的独立版本，可独立运行。
支持与其他接口程序并发运行。
...
logger = get_logger("format.index_relationship_mapping_fetch_and_save_belonging_indices")
...
def main() -> None:
    """
    主函数：通过查询数据库股票列表，调用所属指数接口获取并存储数据。
